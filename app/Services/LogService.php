...
class LogService
{
    ...
    public static function report($str,$name=LogService::LOG_REPORT, $day = 30)
    {
        if (empty(self::$loggers[$name])) {
            self::$loggers[$name] = new Logger($name);
            $handler = (new RotatingFileHandler(storage_path("logs/" . $name . ".log"), $day))
                ->setFormatter(new LineFormatter("%message% %context% %extra%\n", "", false, true));
